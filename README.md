...
- **🔗 内容感知打开** —— 对 URL / 邮箱 / 文件路径 / 文件条目，按 `O` 或**右键**一键打开（浏览器 / 默认邮箱 / 资源管理器）；触发键可在设置里改（支持鼠标中键或关闭）。
- **👁️ 实时预览 + 信息条** —— 鼠标悬浮或按空格弹出预览；顶部信息条显示**类型 · 字数 · 时间 · 可打开提示**，长文可滚动，鼠标侧键 `M4 / M5` 翻页。
- **🕒 相对时间** —— 卡片显示「刚刚 / N 分钟前 / 昨天 …」，完整时间戳在预览信息条中可见。
- **🔍 搜索 + 筛选** —— 搜索支持大小写 `Aa`、全词 `W`；按类型 / 收藏快速过滤。
- **📌 置顶 · ⭐ 收藏 · 🗑️ 删除** —— 键盘、**点击卡片右上角图标**、或逐条管理常用内容。
- **⌨️ 全键盘 + 双击回贴** —— **双击卡片**或 `Enter` 回贴到原窗口，`Shift` 走纯文本，数字键 `1–5` 秒贴，`Home / End` 跳首尾。
- **🌗 亮 / 暗双主题** & **🌍 中英双语** —— 一键切换（默认暗色 / 跟随系统语言），卡片、色块、缩略图、图标全部自适应。
- **🔄 检查更新** —— 内置检查更新：发现新版本可下载、SHA-256 校验后自替换并重启（Windows）。
